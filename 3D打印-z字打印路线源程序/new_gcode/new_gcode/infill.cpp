...
void generateLineInfill(const Polygons& in_outline, Polygons& result, int extrusionWidth, int lineSpacing, int infillOverlap, double rotation)
{
    ...
    boundary.mmin.X = ((boundary.mmin.X / lineSpacing) - 1) * lineSpacing;
    int lineCount = (boundary.mmax.X - boundary.mmin.X + (lineSpacing - 1)) / lineSpacing;
    ...
    for(unsigned int polyNr=0; polyNr < outline.size(); polyNr++)
    {
        ...
        for(unsigned int i=0; i < outline[polyNr].size(); i++)
        {
            ...
            int idx0 = (p0.X - boundary.mmin.X) / lineSpacing;
            int idx1 = (p1.X - boundary.mmin.X) / lineSpacing;
            int64_t xMin = p0.X, xMax = p1.X;
            if (p0.X > p1.X) { xMin = p1.X; xMax = p0.X; }
            if (idx0 > idx1) { int tmp = idx0; idx0 = idx1; idx1 = tmp; }
            for(int idx = idx0; idx<=idx1; idx++)
            {
                int x = (idx * lineSpacing) + boundary.mmin.X + lineSpacing / 2;
                if (x < xMin) continue;
                if (x >= xMax) continue;
	...
	int idxx = 0;
	for (int64_t x = boundary.mmin.X + lineSpacing / 2; x < boundary.mmax.X; x += lineSpacing)
	{
		idxx += 1;
	}
    int idx = 0;
    ...
    for(int64_t x = boundary.mmin.X + lineSpacing / 2; x < boundary.mmax.X; x += lineSpacing)
    {
		...
		if (idx >= 1)
		{
			...
			这里要拿到上一根的最后一个点坐标
			*/
			qsort(cutList[idx-1].data(), cutList[idx-1].size(), sizeof(int64_t), compare_int64_t);
			for (unsigned int i = 0; i + 1 < cutList[idx - 1].size(); i += 2)
			{
				if (cutList[idx-1][i + 1] - cutList[idx-1][i] < extrusionWidth / 5)
					continue;
				if ((idx-1) % 2 != 0)
				{
					a = matrix.unapply(Point(x - lineSpacing, cutList[idx - 1][lasi + 1]));
					line = cutList[idx - 1][lasi + 1];
				}
				else
				{
					a = matrix.unapply(Point(x - lineSpacing, cutList[idx - 1][lasi]));
					line = cutList[idx - 1][lasi];
				}
        ...
        for(unsigned int i = 0; i + 1 < cutList[idx].size(); i+=2)
        {
			...
			*/
			...
			{
				ClipperLib::Polygon p2;
				if (idx % 2 != 0&& cutList[idx][i]!=line)
				{
					if (line - cutList[idx][i] > extrusionWidth / 5)
					{
						p2.push_back(a);
						p2.push_back(matrix.unapply(Point(x, cutList[idx][i])));
						result.add(p2);
					}
				...
				if(idx%2==0 && cutList[idx][i] != line)
				{
					if (line - cutList[idx][i+1] > extrusionWidth / 5)
					{
						p2.push_back(a);
						p2.push_back(matrix.unapply(Point(x, cutList[idx][i + 1])));
						result.add(p2);
					}
